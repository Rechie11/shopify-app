...
6-bottle tasting flight and gets live feedback on whether the flight is *balanced* across heat
...
3. **Bundle performance is unknowable.** Shopify Analytics reports on products and orders, not
   on *bundles as a merchandising object*. The merchant can't tell whether the flight that took
...
*Tradeoff, stated plainly:* a table-based queue polls, so it burns a small amount of DB I/O when
...
await fetch('/cart/add.js', {
  ...
  body: JSON.stringify({ items: selections.map((s, i) => ({
    id: s.variantId, quantity: 1,
    properties: {
...
*Tradeoff:* the discount only becomes visible at cart/checkout, so the builder labels its number
...
- Concept, decision log, tradeoffs, roadmap → **`APP_DECISIONS.md`** *(submission deliverable)*
