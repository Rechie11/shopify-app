...
*Goal: `npm run dev` opens an embedded app inside a real dev store.*
...
*Goal: the plumbing that everything else stands on is done and tested.*
...
*Goal: the full create → edit → publish loop works end to end.*
...
*Goal: the feature that makes this more than CRUD.*
...
*Goal: a storefront that looks designed, not assembled.*
...
*Goal: the two halves become one system.*
...
2. Alert acknowledge/resolve UI — keep alert *generation*, drop the workflow
...
7. `flavor-matrix` and `batch-story` both, leaving three custom sections exactly
8. Playwright tests — keep the vitest unit tests on scoring
9. Traction component of the score — ship with three components and renormalised weights, and
   say so
...
| 5:15 | **The score** | Open the at-risk bundle. Read the reason out loud: *six days of cover on Charred Pineapple.* Then the recommendation. **This is the moment. Slow down here** |
