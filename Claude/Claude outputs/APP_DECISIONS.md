...
flight and gets live feedback on whether the flight is *balanced* across heat and flavour, with a
...
unique index *is* the idempotency mechanism — insert first and let the duplicate-key error tell
you it is a replay.
...
2. **Forecasting instead of trailing velocity.** Days-of-cover currently uses a flat 7-day
   ...
   would move the inventory score from *reactive* to *predictive* — telling a merchant a bundle
